...
# phab_rasts <- rasterFromXYZ(p_habitat) # this makes most computers hang as quite large rasters
...
phab_rasts  <- stack(pmacroalgae, psand, preef, pseagrass, 
                     psponges, prock, pbiogenic)
phab_rasts  <- raster::aggregate(phab_rasts, fact = 10, fun = mean)             #scale to 40 x 40 res
plot(phab_rasts)

writeRaster(phab_rasts, "output/multibeam_habitat_fssgam/layer.tif", 
            bylayer = TRUE, suffix = names(phab_rasts))
...
# phab_df     <- as.data.frame(phab_rasts, xy = TRUE, na.rm = TRUE)
# 
...
# # split and output data (whole data are too large for git agh) - this was solved via the aggregation above
