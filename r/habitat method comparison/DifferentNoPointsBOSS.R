...
# 
# Another way to summarise data:
#   se <- function(x) sd(x)/sqrt(length(x))
#   
#   library(dplyr)
# grouped <- group_by(sample.size,class)
# summarise(grouped, mean=mean(value), sd=sd(value), se=se(value))
# 
...
# bsd <- aggregate(measurement~Class, data = BOSSlong, sd)
head(bse)
head(bsd)

#Use this if you want se
...
# #Use this if you want sd
# b <- cbind(bav, se = bsd[,2])
# head(b)
# b$lower <- b$measurement-b$se
# b$upper <- b$measurement+b$se
# head(b)
...
bsd2 <- aggregate(measurement~Class, data = BOSS100long, sd2)
head(bse2)
head(bsd2)
...
bsd3 <- aggregate(measurement~Class, data = BOSS50long, sd3)
head(bse3)
head(bsd3)
...
bsd4 <- aggregate(measurement~Class, data = BOSS25long, sd4)
head(bse4)
head(bsd4)
