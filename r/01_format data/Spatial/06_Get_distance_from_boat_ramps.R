...
met.bruv <- read.csv("2020_south-west_stereo-BRUVs.checked.metadata.csv")%>%
  mutate(method = 'BRUV')
met.boss <- read.csv("2021-03_West-Coast_BOSS.checked.metadata.csv")%>%
  mutate(method = 'BOSS')
metadata <- bind_rows(met.bruv,met.boss)
...
samples.ramps <- metadata %>% 
  dplyr::select("sample","method", "latitude", "longitude") #this has OpCode,Latitude,Longitude in it
...
distance.to.ramp<-samples.ramps%>%
  ...
  dplyr::select(sample,method,distance.to.ramp)%>%
