...
ggplot(habi, aes(depth, sd.relief)) +     geom_point(alpha = 0.2) + geom_smooth()
ggplot(habi, aes(detrended, sd.relief)) + geom_point(alpha = 0.2) + geom_smooth()
ggplot(habi, aes(roughness, sd.relief)) + geom_point(alpha = 0.2) + geom_smooth()
...
relief_stack   <- inla.stack(data = list(y = traind$sd.relief),
                             A = list(meshadata, 1),
...
ggsave("plots/relief_model/sdreliefmodel_ci.png", width = 5, height = 4, dpi = 160)
...
plot(prelief[[2]])
saveRDS(prelief[[2]], "output/spatial/raster/predicted_sdrelief_raster.rds")
...
saveRDS(pcelldf, 'output/spatial/raster/predicted_sdrelief_site.rds')

ggplot(pcelldf, aes(x, y)) +
  geom_tile(aes(fill = prelief)) +
  scale_fill_viridis() +
  geom_point(data = habi, aes(longitude.1, latitude.1, colour = sd.relief),
             alpha = 4/5, size = 1) +
