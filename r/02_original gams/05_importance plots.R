...
dat.taxa.label<-dat%>%
  ...
  mutate(label=ifelse(predictor=="broad.reef"&resp.var=="smaller than legal size","X",label))%>%
...
gg.importance.scores <- ggplot(dat.taxa.label, aes(x=predictor,y=resp.var,fill=importance))+
  ...
  scale_x_discrete(labels = c("Reef","Macroalgae","Mean relief","SD relief","Depth","Roughness",
